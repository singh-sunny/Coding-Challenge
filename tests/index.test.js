...
import { MAX_IN_PERIOD } from '../constants/index.constants';
//Part 1
describe('calculateEnergyUsageSimple', () => {
  ...
  it('should calculate correctly when the appliance is off the whole time with no events', () => {
    const usageProfile = {
      initial: 'off',
      events: [],
    };
  ...
  it('should calculate correctly when the appliance is off the whole time with all off events', () => {
    const usageProfile = {
      initial: 'off',
      events: [
  ...
  it('should calculate correctly when the appliance is on the whole time with many on events', () => {
    const usageProfile3 = {
      initial: 'on',
      events: [
        { timestamp: 30, state: 'on' },
  ...
  it('should throw all error on malformed profile data', () => {
    let profileData = {
      initial: 'invalid initial state',
      events: [{ timestamp: 100, state: 'on' }],
    };

    expect(() => calculateEnergyUsageForDay(profileData, 5)).toThrow(
      ErrorMessages.INVALID_PROFILE_INITIAL_STATE
    );
    ...
    expect(() => calculateEnergyUsageForDay(profileData, 5)).toThrow(
      ErrorMessages.MISSING_PROFILE_INITIAL_STATE
    );
    ...
    expect(() => calculateEnergyUsageForDay(profileData, 5)).toThrow(
      ErrorMessages.MISSING_PROFILE_EVENTS
    );

    profileData = {
      initial: 'off',
      events: [{ timestamp: -1, state: 'off' }],
    };

    expect(() => calculateEnergyUsageForDay(profileData, 5)).toThrow(
      ErrorMessages.INVALID_TIMESTAMP_IN_EVENT
    );
    ...
    expect(() => calculateEnergyUsageForDay(profileData, 5)).toThrow(
      ErrorMessages.MISSING_TIMESTAMP_IN_EVENT
    );
    ...
    expect(() => calculateEnergyUsageForDay(profileData, 5)).toThrow(
      ErrorMessages.MISSING_STATE_IN_EVENT
    );

    profileData = {
      initial: 'off',
      events: [{ timestamp: 100, state: 'invalid state' }],
    };

    expect(() => calculateEnergyUsageForDay(profileData, 5)).toThrow(
      ErrorMessages.INVALID_DEVICE_STATE_IN_EVENT
    );
...
describe('calculateEnergySavings', () => {
  ...
  it('should throw all error on malformed profile data', () => {
    ...
    profileData = {
      events: [{ timestamp: 100, state: 'auto-off' }],
    };
